...
class TokenEndPointSpec extends FlatSpec with ScalaFutures {
  ...
  def successfulDataHandler() = new MockDataHandler() {
    ...
    override def createAccessToken(authInfo: DefaultAuthInfo[User]): Future[AccessToken] = Future.successful(AccessToken("token1", None, Some("all"), Some(3600), new Date()))
  ...
  it should "be Failure when DataHandler throws Exception" in {
    ...
    def dataHandler = new MockDataHandler() {
      ...
      override def createAccessToken(authInfo: DefaultAuthInfo[User]): Future[AccessToken] = throw new Exception("Failure")
